...
// index fields 'user' & 'phoneNo' for faster new account validation //
	accounts.createIndex({user: 1, phoneNo: 1});
}
...
module.exports.generatePasswordKey = function(phoneNo, ipAddress, callback)
{
	let passKey = guid();
	accounts.findOneAndUpdate({phoneNo:phoneNo}, {$set:{
		ip : ipAddress,
		passKey : passKey
...
module.exports.addNewAccount = function(newData, callback)
{
	accounts.findOne({user:newData.user}, function(e, o) {
		if (o){
			...
			accounts.findOne({phoneNo:newData.phoneNo}, function(e, o) {
				if (o){
					callback('phoneNo-taken');
				}	else{
